...
class RealTimeSubtitleService : LifecycleService() {
    ...
    private fun initializeSpeechRecognizer() {
        try {
            ...
            Log.e(TAG, "Error al inicializar el reconocimiento de voz: ${e.message}")
        }
    }
    ...
    private fun handleSpeechResults(results: Bundle?, isPartial: Boolean) {
        results?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.let { resultList ->
            if (resultList.isNotEmpty()) {
                ...
                Log.d(TAG, "${if (isPartial) "Parcial" else "Final"}: $detectedText")
    ...
    private fun broadcastSubtitle(subtitle: String) {
        serviceScope.launch {
            try {
                ...
                Log.e(TAG, "Error al emitir subtítulo: ${e.message}")
            }
        }
    }
    ...
    private fun startListening() {
        try {
            ...
            Log.e(TAG, "Error al iniciar la escucha: ${e.message}")
        }
    }
    ...
    private fun restartListening() {
        try {
            ...
            Log.e(TAG, "Error al reiniciar la escucha: ${e.message}")
        }
    }
    ...
    override fun onDestroy() {
        ...
        if (::speechRecognizer.isInitialized) {
            try {
                ...
                Log.e(TAG, "Error al destruir el reconocedor: ${e.message}")
            }
        }
    }
